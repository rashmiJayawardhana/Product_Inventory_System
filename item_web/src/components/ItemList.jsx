...
function ItemList() {
  ...
  const columns = useMemo(() => [
    ...
    { Header: "Name", accessor: "name", className: "w-60 px-5 py-3" },
    { Header: "Description", accessor: "description", className: "w-60 px-5 py-3" },
    { 
      Header: "Quantity",
      accessor: "quantity",
      className: "w-44 px-5 py-3",
      Cell: ({ row }) => (
        <input
          type="number"
          value={row.original.quantity || 1}
          onChange={(e) => handleQuantityChange(row.original.item_id, Number(e.target.value))}
          className="w-10 text-center border border-gray-300 rounded-md"
        />
      )
    },
    { Header: "Price", accessor: "price", className: "w-44 px-5 py-3" },
    { 
      Header: "Actions", 
      id: "actions", 
      accessor: "actions",
      ...
      Cell: ({ row }) => (
        <div className="flex gap-2 px-2 py-3">    
          ...
          <button 
            ...
            <div><LiaUserTimesSolid size={20} /></div>
            <div>Delete</div>
          </button>
        </div>               
      )
